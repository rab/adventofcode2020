...
class Passport
  attr_accessor :byr, # byr (Birth Year) - four digits; at least 1920 and at most 2002.
  ...
  def valid?
    (1920..2002).include?(self.byr) &&
      (2010..2020).include?(self.iyr) &&
      (2020..2030).include?(self.eyr) &&
      self.hgt.is_a?(Array) &&
      ['cm','in'].include?(self.hgt.last) &&
      (self.hgt.last == 'cm' ? (150..193) : (59..76)).cover?(self.hgt.first) &&
      /\A\#[0-9a-f]{6}\z/.match?(self.hcl) &&
      %w[amb blu brn gry grn hzl oth].include?(self.ecl) &&
      /\A\d{9}\z/.match?(self.pid)
  end

  def to_s
    "Passport:#{self.pid.inspect} byr:#{self.byr || 'BAD'} iyr:#{self.iyr || 'BAD'} eyr:#{self.eyr || 'BAD'} hgt:#{self.hgt || 'BAD'} hcl:#{self.hcl || 'BAD'} ecl:#{self.ecl || 'BAD'} cid:#{self.cid || '(unknown)'} #{self.valid? ? 'VALID' : self.simpe? ? 'SIMPLE' : 'BAD'}"
  end
end

passports = []
input.each_line('', chomp: true) do |line|
  passport = Passport.new(Hash[line.scan(/([a-z]{3}):(\S+)/)])
  unless passport.valid?
    puts line, passport; puts '-'*80
  end if debugging
  passports << passport
end

puts passports.reject(&:valid?) if testing || debugging

puts "Total: #{passports.size}"
puts "Part 1:", passports.select(&:simple?).count
puts "Part 2:", passports.select(&:valid?).count
