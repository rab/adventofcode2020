...
#   connect to it directly would need to have a joltage rating of 1, 2, or 3 jolts. Of these, only
#   one you have is an adapter rated 1 jolt (difference of 1).
...
puts joltages.inspect if debugging
joltages.sort!
puts joltages.inspect if debugging
joltages.unshift 0
puts joltages.inspect if debugging
...
part1 = joltages.each_cons(2).map {|(l,h)| h-l }.partition {|d| d==1 }.map(&:count).reduce(:*)
puts "Part 1:", part1
expected = 220
raise "Wrong! expected #{expected}, got #{part1}" if testing && expected != part1
...
options = Hash.new {|h,n| h[n] = h[n-3] + h[n-2] + h[n-1] }
options.update(1 => 1, 2 => 1, 3 => 2)

(1..7).each do |i| puts options[i]; end if testing

runs = [1]
last = joltages.shift
while joltages.first
  if joltages.first - last == 1
    runs[-1] += 1
  else
    runs << 1
  end
...
part2 = runs.map {|n| options[n] }.reduce(:*)
puts "Part 2:", part2
raise "Wrong! expected #{expected}, got #{part2}" if testing && expected != part2
