...
class Program
  attr_accessor :instructions, :pc, :acc
  ...
  def <<(instr)
    op, val = /\A(\w{3}) ([+-]\d+)\z/.match(instr).captures
    self.instructions << {op: op, val: val.to_i, cnt: 0}
  end

  def step(n=1)
    n.times do |i|
       self.instructions[self.pc][:cnt] += 1
      case self.instructions[self.pc][:op]
      when 'nop'
        self.pc += 1
      when 'acc'
        self.acc += self.instructions[self.pc][:val]
        self.pc += 1
      when 'jmp'
        self.pc += self.instructions[self.pc][:val]
      end
    end
    # puts self.acc
  end
  ...
  def run
    step until halted? || infinite?
  end

  def exec_cnt_at_pc
    self.instructions[self.pc][:cnt]
  end

  def infinite?
    self.pc < self.instructions.size && exec_cnt_at_pc.nonzero?
  end

  def halted?
    @halted ||= self.pc == self.instructions.size
  end

  def to_s
    self.instructions.map.with_index {|inst_hsh,idx|
      "%<idx>4d: %<op>3s %<val>+4d | %<cnt>3d"%(inst_hsh.merge(idx: idx))
    }.join("\n") + "pc: #{self.pc}  acc: #{self.acc}\n" + (infinite? ? " INF!" : halted? ? " HALT" : "")
  end

  def alter(location)
    variant = self.class.new
    self.instructions.each do |instr_hsh|
      variant.instructions << { op:  instr_hsh[:op].dup, val: instr_hsh[:val].dup, cnt: 0 }
    end
    case variant.instructions[location][:op]
    when 'jmp'
      variant.instructions[location][:op] = 'nop'
    when 'nop'
      variant.instructions[location][:op] = 'jmp'
    end
    variant
  end
end
...
puts program if debugging || testing

program.step until program.infinite?

puts "Part 1:", program.acc
...
variant = nil
program.instructions.size.times do |location|
  next if program.instructions[location][:op] == 'acc'
  variant = program.alter(location)
  variant.run
  if debugging
    puts variant if testing
    puts "#{location}: #{variant.halted? ? 'HALT' : variant.infinite? ? 'INF!' : '?'}"
    puts "\n"*2
  end
  break if variant.halted?
end

puts "Part 2:", variant.acc
